...
cmd_check() {
  ...
  echo "  • Play Console app for 'social.betty' (first upload is manual; enroll in Play App Signing)"
  echo "  • Google sign-in: register an Android OAuth client (package social.betty) with these SHA-1s:"
...
cmd_setup() {
  ...
  echo "  1. Create the Play Console app for 'social.betty' and upload one AAB by hand (accept the"
  echo "     developer agreement, enroll in Play App Signing). After that the workflow publishes."
  echo "  2. Register an Android OAuth client (package social.betty) with SHA-1:"
