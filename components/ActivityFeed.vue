  ...
  <div>
    <transition name="clear" tag="div">
      <div v-if="list.length > 0" class="clear-all-container">
        <div class="clear-all" @click="clearAll" @mouseenter="hoverClear = true" @mouseleave="hoverClear = false">
      ...
      <div v-for="message in list" :key="message.id" class="feed-item">
        <div class="row row--center-v">
          ...
          <template v-else-if="message.type === 'user_exact_score'">
            <div class="column column--wrap column--no-padding">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-star feed-item__icon">
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>
              </svg>
            </div>
            <div class="column">
              <exact-score-list-item :message="message.message"></exact-score-list-item>
            </div>
          </template>
          <template v-else>
...
export default {
  ...
  methods: {
    clearAll() {
      this.$store.dispatch('message/clearAll');
    },
  },
};
</script>
